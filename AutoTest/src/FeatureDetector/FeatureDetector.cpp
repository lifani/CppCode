...
bool FileKeyPointsOut(FILE* pf, vector<KeyPoint>& vKeyPoint)
{
    ...
    {
        fprintf(pf, "\n%f\t%f\t%0.14f", itr->pt.x, itr->pt.y, itr->response);
    }
...
bool FileKeyPointsIn(FILE* pf, long& pos, vector<KeyPoint>& vKeyPoint)
{
    ...
    {
        KeyPoint p;
        fscanf_s(pf, "%d %d %0.14f", &p.pt.x, &p.pt.y, &p.response);
...
int CFeatureDetector::StartTest( const string& strIn, const string& strOut )
{
